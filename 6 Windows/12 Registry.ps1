	      ...
	      reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\PersonalizationCSP" /v "LockScreenImageStatus" /t REG_DWORD /d "1" /f | Out-Null
				...
				Get-CimInstance Win32_StartupCommand | ForEach-Object {
				    ...
				    foreach ($r in 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\Run',
				        ...
				        if (Get-ItemProperty -Path $r -Name $n -ErrorAction SilentlyContinue) {
				            Set-ItemProperty -Path $r -Name $n -Type Binary -Value ([byte[]](0x03,0,0,0,0,0,0,0))
				        }
				    }
				}
				...
				if (Test-Path "C:\Program Files\VMware\VMware Tools\vmtoolsd.exe") { 
					...
					# add / refresh entry in standard Run key
					Set-ItemProperty -Path $runKey -Name "VMware User Process" `
				    	-Value '"C:\Program Files\VMware\VMware Tools\vmtoolsd.exe" -n vmusr' -Force
					# re-enable in StartupApproved (binary: 02 = enabled)
					New-ItemProperty -Path $approvedKey -Name "VMware User Process" `
				    	-PropertyType Binary -Value ([byte[]](0x02,0,0,0,0,0,0,0)) -Force | Out-Null
				}
			
				# unlock all power settings options
				Get-ChildItem 'HKLM:\SYSTEM\CurrentControlSet\Control\Power\PowerSettings' -Recurse |
				    ForEach-Object {
				        if (Get-ItemProperty $_.PSPath -Name Attributes -ErrorAction SilentlyContinue) {
				            Set-ItemProperty $_.PSPath -Name Attributes -Value 0
				        }
				    }
			  ...
			  Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control" -Name "SvcHostSplitThresholdInKB" -Type DWord -Value $ram -Force
				...
				foreach ($gpu in $gpuDevices) {
					...
					reg add "HKLM\SYSTEM\ControlSet001\Enum\$instanceID\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties" /v "MSISupported" /t REG_DWORD /d "1" /f | Out-Null -ErrorAction SilentlyContinue
				}
			
				# allow double-click of .ps1 files
				$regPath = 'Registry::HKEY_CLASSES_ROOT\Applications\powershell.exe\shell\open\command'
				if (-not (Test-Path $regPath)) {
			    	New-Item -Path $regPath -Force | Out-Null
				}
				Set-ItemProperty -Path $regPath -Name '(default)' `
